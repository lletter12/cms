...
import {Dimensions, ElementProps} from "@/utils/types/element";
import {RootContext} from "@/components/types/menu/menu";
import {getFocusElement} from "@/utils/getFocusElement";
...
import {
    ...
    buildCellMap, findNonDisabledIndex, getCellIndexOfCorner, getCellIndices,
...
export function useListNavigation(
 ...
 /*   if (__DEV__) {
    ...
    const floatingFocusElement = getFocusElement(elements.floating);
    ...
    const focusItemOnOpenRef = React.useRef<string | boolean>(focusItemOnOpen);
            ...
            if (
                ...
                const waitForListPopulated = () => {
                    if (listRef.current[0] == null) {
                        ...
                        indexRef.current =
                            keyRef.current == null ||
                            isMainOrientationToEndKey(keyRef.current!, orientation as UseListNavigationProps['orientation'], rtl) ||
                            nested
    ...
    const commonOnKeyDown = useEffectEvent((event: React.KeyboardEvent) => {
        ...
        if (
            nested &&
            isCrossOrientationCloseKey(event.key, orientation as UseListNavigationProps['orientation'], rtl, cols)
        ) {
            stopEvent(event);
                    ...
                    },
                    {
                        ...
                        disabledIndices: getCellIndices(
                            [
                                ...
                                // disabledIndices가 undefined일 경우 빈 배열로 처리
                            ],
                            cellMap
                        ),
                        minIndex: minGridIndex ?? 0, // minIndex가 undefined일 경우 0으로 처리
                        maxIndex: maxGridIndex ?? 0, // maxIndex가 undefined일 경우 0으로 처리
                        prevIndex: getCellIndexOfCorner(
                            indexRef.current > (maxGridIndex ?? 0) ? (minGridIndex ?? 0) : indexRef.current,
                            sizes,
                            cellMap,
                            cols,
        ...
        if (isMainOrientationKey(event.key, orientation as UseListNavigationProps['orientation'])) {
            stopEvent(event);
            ...
            if (isMainOrientationToEndKey(event.key, orientation as UseListNavigationProps['orientation'], rtl)) {
                if (loop) {
                    indexRef.current =
    ...
    const reference: ElementProps['reference'] = React.useMemo(() => {
        ...
        return {
            ...
            onKeyDown(event) {
                ...
                const isMainKey = isMainOrientationKey(event.key, orientation as UseListNavigationProps['orientation']);
