...
import {
    ...
    getTarget,
    isTypeAbleCombobox, isVirtualClick, isVirtualPointerEvent,
    stopEvent
} from "@/utils/dom";
...
import {getNextTabbable, getPreviousTabbable, getTabbableOptions, isOutsideEvent, tabbable} from "@/utils/tabbable";
import {FocusManagerProps} from "@/components/types/focus";
...
import {useLiteMergeRefs} from "@/hooks/useLiteMergeRefs/useLiteMergeRefs";
import {useLayoutEffect} from "react";
import {OpenChangeReason} from "@/components/types/menu/menu";
import {addPreviouslyFocusedElement, getFirstTabbableElement} from "@/utils/focusManager";
...
export const FocusManager = (
    ...
    const startDismissButtonRef = React.useRef<HTMLButtonElement|null>(null);
    const endDismissButtonRef = React.useRef<HTMLButtonElement|null>(null);
    const preventReturnFocusRef = React.useRef(false);
    const isPointerDownRef = React.useRef(false);
    const tabbableIndexRef = React.useRef(-1);

    const isInsidePortal = portalContext != null;
    const floatingFocusElement = getFocusElement(floating);
        ...
        function handleFocusOutside(event: FocusEvent) {
            ...
            queueMicrotask(() => {
                ...
                const movedToUnrelatedNode = !(
                    ...
                    relatedTarget && relatedTarget!.hasAttribute(createAttribute('focus-guard')) ||
                    (tree &&
                            ...
                            getAncestors(tree.nodesRef.current, nodeId).find(
                                (node) =>
                                    [
                                        ...
                                        getFocusElement(node.context?.elements.floating),
    ...
    React.useEffect(() => {
        if (disabled) return;
        if (!floating) return;
        const selector: string = `[${createAttribute('portal')}]`;
        const portalNodes = Array.from(
            (portalContext?.portalNode as Element)?.querySelectorAll(selector) || [],
        );
    ...
    useLayoutEffect(() => {
        ...
        function getReturnElement() {
            ...
            return returnFocusRef.current || fallbackEl;
        }
